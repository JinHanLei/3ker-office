...
if (args.includes("--help") || !args.length) {
  ...
  const take = (key: string, defaultValue?: string) => {
    const at = args.indexOf(key);
    return at < 0 ? defaultValue : args.splice(at, 2)[1];
  };
  const json = args.includes("--json");
  if (json) args.splice(args.indexOf("--json"), 1);
  const runId = take("--run"),
    db = take("--db", ".runtime/manual/run.sqlite")!,
    scenario = take("--scenario", "scenarios/synthetic/kernel-smoke")!,
    actorId = take("--actor", "player")!,
    accountId = take("--account", "player")!;
  ...
  const playback = new Playback(k, (e) => {
    console.error(e);
    process.exitCode = 1;
  });
  const print = (result: unknown) =>
    console.log(
      json ? stableStringify(result) : `结果：${stableStringify(result)}`,
    );
    ...
    if (args[0] === "create")
      print(
        await k.execute({
          ...
          actorId,
          accountId,
          commandId: "initial-capital",
          type: "CreateAccount",
          initialCashFen: decimalUnits(args[1] ?? "100000", 2).toString(),
        }),
      );
      ...
      try {
        for await (const line of rl) {
          ...
          try {
            print(await dispatch(k, tokens, context));
          } catch (e) {
      ...
      if (args[0] === "play")
        throw Error("play 需要 interactive 会话；脚本请使用 advance/step");
      const result = await dispatch(k, args, context);
      print(result);
      if (
        result &&
