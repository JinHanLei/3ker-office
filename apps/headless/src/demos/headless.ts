...
const root = join(".runtime", "headless-" + randomUUID()),
  runId = "headless";
const k = await openKernel({
  db: join(root, "run.sqlite"),
  runId,
  scenario: "scenarios/synthetic/kernel-smoke",
  create: true,
});
const receipts: Receipt[] = [];
async function exec(command: GameCommand, expected?: string) {
  const r = await k.execute(command);
  receipts.push(r);
  if (expected ? r.code !== expected : !r.ok) throw Error(JSON.stringify(r));
  return r;
}
const base = { runId, actorId: "player", accountId: "player" };
try {
  for (const accountId of ["player", "test-a", "test-b"])
    await exec({
      ...base,
      accountId,
      commandId: "create-" + accountId,
      type: "CreateAccount",
      initialCashFen: "10000000",
    });
  const initialSearch = k.query({ type: "search" });
  await exec({
    ...base,
    commandId: "watch",
    type: "AddWatchlist",
  ...
  const s = k.exportSnapshot("player"),
    a = s.accounts.player!;
  if (a.availableCashFen !== 10013890n || a.frozenCashFen !== 0n)
    throw Error("Unexpected demo cash");
  exportRun(k, root, "player", {
    marketDataKind: "synthetic",
    rulesetKind: "game-test",
    initialSearch,
    finalSearch: k.query({ type: "search" }),
    receipts,
    expectedCashFen: "10013890",
    actualCashFen: a.availableCashFen.toString(),
  });
  await k.checkpoint();
  console.log(
