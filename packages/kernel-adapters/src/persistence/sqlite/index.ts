...
export class SqliteRunStore implements RunStore {
  ...
  async load(runId: string): Promise<RunState | null> {
    const row = this.db
      .prepare("SELECT payload,sha256 FROM snapshots WHERE run_id=?")
      .get(runId) as { payload: string; sha256: string } | undefined;
    if (!row) return null;
    if (createHash("sha256").update(row.payload).digest("hex") !== row.sha256)
      throw new Error("Corrupt snapshot checksum");
    const s = validateRunState(decodeState<RunState>(row.payload));
    if (s.runId !== runId) throw new Error("Invalid saved run identity");
    return s;
  }
  async commit(s: RunState, expected: number): Promise<void> {
    ...
    this.db
      .transaction(() => {
        const old = this.db
          .prepare("SELECT state_version FROM runs WHERE run_id=?")
          .get(s.runId) as { state_version: number } | undefined;
        if ((old?.state_version ?? -1) !== expected)
          throw new KernelError("STATE_VERSION_CONFLICT");
        if (s.stateVersion !== expected + 1)
          throw new Error("Invalid next storage revision");
        this.db
          .prepare(
            "INSERT INTO runs VALUES(?,?) ON CONFLICT(run_id) DO UPDATE SET state_version=excluded.state_version",
          )
          .run(s.runId, s.stateVersion);
        const payload = encodeState(s),
          hash = createHash("sha256").update(payload).digest("hex");
        this.db
          .prepare(
            "INSERT INTO snapshots VALUES(?,?,?,?) ON CONFLICT(run_id) DO UPDATE SET state_version=excluded.state_version,payload=excluded.payload,sha256=excluded.sha256",
          )
          .run(s.runId, s.stateVersion, payload, hash);
        const insertEvent = this.db.prepare(
            "INSERT OR IGNORE INTO events VALUES(?,?,?)",
          ),
          insertLedger = this.db.prepare(
            "INSERT OR IGNORE INTO ledger_entries VALUES(?,?,?)",
          ),
          insertReceipt = this.db.prepare(
            "INSERT OR IGNORE INTO command_receipts VALUES(?,?,?)",
          );
        for (const event of s.events)
          insertEvent.run(s.runId, event.eventSeq, encodeState(event));
        for (const entry of s.ledger)
          insertLedger.run(s.runId, entry.eventSeq, encodeState(entry));
        for (const [id, receipt] of Object.entries(s.receipts))
          insertReceipt.run(s.runId, id, encodeState(receipt));
