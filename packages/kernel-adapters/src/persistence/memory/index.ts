...
export class MemoryRunStore implements RunStore {
  ...
  async commit(state: RunState, expected: number): Promise<void> {
    if ((this.runs.get(state.runId)?.stateVersion ?? -1) !== expected)
      throw new KernelError("STATE_VERSION_CONFLICT");
    if (state.stateVersion !== expected + 1)
      throw Error("Invalid next storage revision");
